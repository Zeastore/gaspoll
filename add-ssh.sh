...
stunnel5port="$(cat ~/log-install.txt | grep -w "PORT STUNNEL" | cut -d: -f2|sed 's/ //g')"
...
ssl="$(cat ~/log-install.txt | grep -w "PORT STUNNEL" | cut -d: -f2)"
...
echo -e "OVPN TCP : http://$MYIP:89/client-tcp-1194.ovpn"
echo -e "OVPN UDP : http://$MYIP:89/client-udp-2200.ovpn"
echo -e "OVPN SSL : http://$MYIP:89/client-tcp-ssl.ovpn"
